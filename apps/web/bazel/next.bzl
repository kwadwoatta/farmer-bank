...
def next(
    ...
    ```
    load("//apps/web/bazel:next.bzl", "next")

    next(
        name = "next",
        srcs = [
            "//apps/apps/web/pages",
            "//apps/apps/web/public",
            "//apps/apps/web/styles",
        ],
        data = [
            "//:node_modules/next",
    ...
    in an `apps/apps/web/BUILD.bazel` file where the root `BUILD.bazel` file has
    ...
    ```
    ibazel run //apps/web:next_start
    ```
    ...
    ```
    bazel build //apps/web:next_export
    ```
